...
city <- "bilbao"
color1 <- "sienna" #rx5d
color2 <- "darkorange" #base
color3 <- "darkgoldenrod1" #rx1d
...
# daily_df_neg <- data.frame(year = years, month = months, day = days, date=dates, tp = ncvar_get(tp_d, "tp"))
# daily_df <- daily_df_neg %>% mutate(tp = ifelse(tp < 0, 0, tp))
daily_df_neg <- data.frame(year = years, month = months, day = days, date=dates, tp = ncvar_get(tp_d, "tp")*1000/20)
daily_df <- daily_df_neg %>% mutate(tp = ifelse(tp < 0, 0, tp))
...
monthly_df <- data.frame(year = years, month = months, date=dates, tp = ncvar_get(tp_m, "tp")*1000/20, rx1d = ncvar_get(rx1d, "tp")*1000/20, rx5d = ncvar_get(rx5d, "tp")*1000/20)
...
plot(NA, xlim = range(daily_df$tp), ylim = c(0, 0.5), xlab = "tp", ylab = "Density", main = "")
...
plot(NA, xlim = range(monthly_df$tp), ylim = c(0, 0.003), xlab = "tp", ylab = "Density", main = "")
