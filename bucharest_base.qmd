...
monthly_df_neg <- data.frame(year = years, month = months, date=dates, tp = ncvar_get(tp_m, "tp")*1000/6, rx1d = ncvar_get(rx1d, "tp")*1000/6, rx5d = ncvar_get(rx5d, "tp")*1000/6)
monthly_df <- monthly_df_neg %>% mutate(tp = ifelse(tp < 0, 0, tp))
...
#, color = color2, fill = color2) +
  ...
  scale_color_manual(values = c(summer_color, spring_color, autumn_color, winter_color)) +  # Custom color for each season
  theme_minimal()
```
...
# Create an empty plot
plot(NA, xlim = range(monthly_df$tp), ylim = c(0, 0.008), xlab = "tp", ylab = "Density", main = "")
